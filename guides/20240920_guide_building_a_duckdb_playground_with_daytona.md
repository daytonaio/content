...
This is a comprehensive hands-on guide in using [DuckDB](/definition/20240922_definition_duckdb.md) database to perform a real-world data project in a containerized [workspace](/definition/20240819_definition_daytona%20workspace.md) using Daytona.
You'll follow me along from setup to actually working with DuckDB cli and even with [Python](/definition/20240820_defintion_python.md) via its Client API. So it's a long ride and you can get a coffee nearby.
...
- Set up a Daytona Workspace with DuckDB [environment](/definition/20240819_definition_development%20environment.md)
- Hands-on practice using DuckDB as a CLI Tool
- Hands-on practice using DuckDB client API with [Python](/definition/20240820_defintion_python.md)
- Conclusion
...
[DuckDB](/definition/20240922_definition_duckdb.md) is a fast in-process data analytical database with support of feature-rich SQL dialect complemented with deep integrations into client APIs. It's designed to perform highly complex queries against large databases in embedded configuration, such as combining tables with hundreds of columns and billions of rows. It's specialized for [online analytical processing (OLAP)](/definition/20240922_definition_online_analytical_processing_olap.md) workloads.

### Features of it

DuckDB has many features that make it stand out among other databases focusing on [OLAP](/definition/20240922_definition_online_analytical_processing_olap.md). Some of the features are:
...
- **Feature-Rich:** DuckDB has some interesting features such as extensive support for SQL complex queries, integrations to languages like [Python](/definition/20240820_defintion_python.md), R and Java, and data can be stored as persistent, single-file databases.
- **Speed:** It's faster as it uses a columnar-vectorized query execution engine which improves performance to run [OLAP](/definition/20240922_definition_online_analytical_processing_olap.md) workloads.
- **Free:** Lastly, it's a free [open source](/definition/20240819_definition_open%20source.md) database system which anyone can use because of its permissive MIT License.

## Setting up Daytona Workspace for DuckDB Playground

Alright, that's enough reading, now let us start writing codes. To do so you will need to set up a DuckDB [environment](/definition/20240819_definition_development%20environment.md) in a [Daytona workspace](/definition/20240819_definition_daytona%20workspace.md). Let’s begin.

### Step 1: Create a GitHub Repository

First head to the GitHub website and create a [repository](/definition/20240819_definition_repository.md) with the name of your choice. For my repository name, I’ll use `playground-duckdb`. The full URL path to the repository is `https://github.com/c0d33ngr/playground-duckdb`
...
The `devcontainer.json` content contains configurations to start your DuckDB environment in a [Daytona workspace](/definition/20240819_definition_daytona%20workspace.md).
...
Run the command below in a new tab of your terminal and follow the prompt instructions. It would ask you for a [workspace](/definition/20240819_definition_daytona%20workspace.md) name to use, choose the default.
...
You can now run this command to open the DuckDB [environment](/definition/20240819_definition_development%20environment.md) in your default [IDE](/definition/20240819_definition_integrated%20development%20environment%20_ide_.md) you choose when installing Daytona (Replace `WORKSPACE-NAME` with the name you used when creating the workspace above, in my case it's `playground-duckdb`).
...
That's it. You have done lots of data tasks using [DuckDB](/definition/20240922_definition_duckdb.md) in a [daytona workspace](/definition/20240819_definition_daytona%20workspace.md).
