...
2. **Blank Workspace**:
```bash
daytona create --blank --name workspacename -t fly
```

3. **With Custom Image**:
```bash
daytona create --custom-image ubuntu:latest --custom-image-user ubuntu --name workspacename -t fly
```

#### Example Usage Scenarios

1. **Creating a Development Workspace with VS Code**:
```bash
daytona create https://github.com/daytonaio/content \
  --name dev-workspace \
  -t fly \
  -i vscode \
  --branch main
...
2. **Multi-project Workspace with Environment Variables**:
```bash
daytona create \
  --multi-project \
...
3. **Custom Development Container**:
```bash
daytona create \
  --name custom-dev \
  -t fly \
...
```bash
daytona workspace stop <workspace-name>
```

#### Starting a Workspace
```bash
daytona workspace start <workspace-name>
```

#### Deleting a Workspace

```bash
daytona workspace delete <workspace-name>
```
